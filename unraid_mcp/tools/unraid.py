...
| `user` | `me` | |
| `live` | `cpu`, `memory`, `cpu_telemetry`, `array_state`, `parity_progress`, `ups_status`, `notifications_overview`, `notifications_warnings`, `network`, `owner`, `server_status`, `display`, `docker_container_stats`, `temperature`, `log_tail` (requires `path=`), `notification_feed`, `plugin_install_updates` | |
...
def register_unraid_tool(mcp: FastMCP) -> None:
    ...
    async def unraid(
        ...
        │                 │ network, owner, server_status, display, docker_container_stats,      │
