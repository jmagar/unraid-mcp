...
class QA {
  ...
  static async load(A) {
    ...
    for (const C of g)
      try {
        return await QA.loadFromPath(C);
      } catch (I) {
  ...
  static async loadFromPath(A) {
    ...
    if (!B)
      try {
        const C = await (await import("./incus-settings-__vite-browser-external-2447137e-DYxpcVy9.js")).readFile(A);
...
class TA {
  ...
  setKittyFlags(A) {
    this.setOption(tA.KITTY_KEYBOARD_FLAGS, A);
  }
  encode(A) {
    ...
    if (this.exports.ghostty_wasm_free_opaque(B), this.exports.ghostty_key_event_set_action(C, A.action), this.exports.ghostty_key_event_set_key(C, A.key), this.exports.ghostty_key_event_set_mods(C, A.mods), A.utf8) {
      const N = new TextEncoder().encode(A.utf8), w = this.exports.ghostty_wasm_alloc_u8_array(N.length);
      new Uint8Array(this.exports.memory.buffer).set(N, w), this.exports.ghostty_key_event_set_utf8(C, w, N.length), this.exports.ghostty_wasm_free_u8_array(w, N.length);
    }
    const I = 32, D = this.exports.ghostty_wasm_alloc_u8_array(I), i = this.exports.ghostty_wasm_alloc_usize(), s = this.exports.ghostty_key_encoder_encode(
      this.encoder,
      C,
      D,
      I,
    ...
    if (s !== 0)
      throw this.exports.ghostty_wasm_free_u8_array(D, I), this.exports.ghostty_wasm_free_usize(i), this.exports.ghostty_key_event_free(C), new Error(`Failed to encode key: ${s}`);
    const t = E.getUint32(i, !0), a = new Uint8Array(this.exports.memory.buffer, D, t).slice();
    return this.exports.ghostty_wasm_free_u8_array(D, I), this.exports.ghostty_wasm_free_usize(i), this.exports.ghostty_key_event_free(C), a;
  }
  dispose() {
...
const yA = class u {
  constructor(A, B, g = 80, E = 24, C) {
    ...
    if (this.viewportBufferPtr = 0, this.viewportBufferSize = 0, this.cellPool = [], this.graphemeBuffer = null, this.graphemeBufferPtr = 0, this.exports = A, this.memory = B, this._cols = g, this._rows = E, C) {
      const D = this.exports.ghostty_wasm_alloc_u8_array(hA);
      if (D === 0)
        throw new Error("Failed to allocate config (out of memory)");
      try {
        const i = new DataView(this.memory.buffer);
        let s = D;
        i.setUint32(s, C.scrollbackLimit ?? 1e4, !0), s += 4, i.setUint32(s, C.fgColor ?? 0, !0), s += 4, i.setUint32(s, C.bgColor ?? 0, !0), s += 4, i.setUint32(s, C.cursorColor ?? 0, !0), s += 4;
        for (let t = 0; t < 16; t++)
          i.setUint32(s, ((I = C.palette) == null ? void 0 : I[t]) ?? 0, !0), s += 4;
        this.handle = this.exports.ghostty_terminal_new_with_config(g, E, D);
      } finally {
        this.exports.ghostty_wasm_free_u8_array(D, hA);
      }
    } else
   ...
   */
  getViewport() {
    const A = this._cols * this._rows, B = A * u.CELL_SIZE;
   ...
   */
  getScrollbackLine(A) {
    const B = this._cols * u.CELL_SIZE;
    ...
    if (g < 0)
      return null;
    const E = [], C = this.memory.buffer, I = new Uint8Array(C, this.viewportBufferPtr, g * u.CELL_SIZE), D = new DataView(C, this.viewportBufferPtr, g * u.CELL_SIZE);
    for (let i = 0; i < g; i++) {
      const s = i * u.CELL_SIZE;
      E.push({
  ...
  parseCellsIntoPool(A, B) {
    const g = this.memory.buffer, E = new Uint8Array(g, A, B * u.CELL_SIZE), C = new DataView(g, A, B * u.CELL_SIZE);
    for (let I = 0; I < B; I++) {
      const D = I * u.CELL_SIZE, i = this.cellPool[I];
...
class bA {
  constructor(A) {
    this.bufferChangeEmitter = new m(), this.terminal = A;
  }
  get active() {
  ...
  get normal() {
    return this._normalBuffer || (this._normalBuffer = new kA(this.terminal, "normal")), this._normalBuffer;
  }
  get alternate() {
    return this._alternateBuffer || (this._alternateBuffer = new kA(this.terminal, "alternate")), this._alternateBuffer;
  }
  get onBufferChange() {
...
class fA {
  ...
  getCell(A) {
    if (!(A < 0 || A >= this._length))
      return A >= this.cells.length ? new sA(
        {
          codepoint: 0,
          fg_r: 204,
          fg_g: 204,
...
class zA {
  ...
  extractModifiers(A) {
    let B = W.NONE;
    return A.shiftKey && (B |= W.SHIFT), A.ctrlKey && (B |= W.CTRL), A.altKey && (B |= W.ALT), A.metaKey && (B |= W.SUPER), B;
  }
  /**
  ...
  handleKeyDown(A) {
    ...
    try {
      if (this.getModeCallback) {
        ...
        this.encoder.setOption(tA.CURSOR_KEY_APPLICATION, i);
      }
      const C = A.key.length === 1 && A.key.charCodeAt(0) < 128 ? A.key.toLowerCase() : void 0, I = this.encoder.encode({
        ...
        mods: g,
        utf8: C
      }), D = new TextDecoder().decode(I);
      A.preventDefault(), A.stopPropagation(), D.length > 0 && this.onDataCallback(D);
    } catch (C) {
...
class VA {
  ...
  cacheLink(A) {
    ...
    if (g) {
      ...
      if (!D) {
        const { start: s, end: t } = A.range, a = `r${s.y}:${s.x}-${t.x}`;
        this.linkCache.set(a, A);
        return;
      }
  ...
  provideLinks(A, B) {
    ...
    for (let I = 0; I < C.length; I++) {
      ...
      if (!this.terminal.wasmTerm)
        continue;
      const t = this.terminal.wasmTerm.getHyperlinkUri(i);
      t && g.push({
        text: t,
        range: s,
        activate: (a) => {
...
const MA = class AA {
  ...
  provideLinks(A, B) {
    ...
    AA.URL_REGEX.lastIndex = 0;
    let I = AA.URL_REGEX.exec(C);
    for (; I !== null; ) {
      let D = I[0];
      const i = I.index;
      let s = I.index + D.length - 1;
      const t = D.replace(AA.TRAILING_PUNCTUATION, "");
      t.length < D.length && (D = t, s = i + D.length - 1), D.length > 8 && g.push({
        text: D,
        range: {
          start: { x: i, y: A },
          end: { x: s, y: A }
        ...
        activate: (a) => {
          (a.ctrlKey || a.metaKey) && window.open(D, "_blank", "noopener,noreferrer");
        }
      }), I = AA.URL_REGEX.exec(C);
    }
    B(g.length > 0 ? g : void 0);
  }
  /**
...
MA.URL_REGEX = /(?:https?:\/\/|mailto:|ftp:\/\/|ssh:\/\/|git:\/\/|tel:|magnet:|gemini:\/\/|gopher:\/\/|news:)[\w\-.~:\/?#@!$&*+,;=%]+/gi;
MA.TRAILING_PUNCTUATION = /[.,;!?)\]]+$/;
let ZA = MA;
const cA = {
  foreground: "#d4d4d4",
  background: "#1e1e1e",
...
class jA {
  constructor(A, B = {}) {
    ...
    this.ctx = g, this.fontSize = B.fontSize ?? 15, this.fontFamily = B.fontFamily ?? "monospace", this.cursorStyle = B.cursorStyle ?? "block", this.cursorBlink = B.cursorBlink ?? !1, this.theme = { ...cA, ...B.theme }, this.devicePixelRatio = B.devicePixelRatio ?? window.devicePixelRatio ?? 1, this.palette = [
      this.theme.black,
      this.theme.red,
      this.theme.green,
      this.theme.yellow,
  ...
  measureFont() {
    ...
    const B = A.measureText("M"), g = Math.ceil(B.width), E = B.actualBoundingBoxAscent || this.fontSize * 0.8, C = B.actualBoundingBoxDescent || this.fontSize * 0.2, I = Math.ceil(E + C) + 2, D = Math.ceil(E) + 1;
  ...
  render(A, B = !1, g = 0, E, C = 1) {
    ...
    const D = A.getCursor(), i = A.getDimensions(), s = E ? E.getScrollbackLength() : 0;
    (I = A.needsFullRedraw) != null && I.call(A) && (B = !0), (this.canvas.width !== i.cols * this.metrics.width * this.devicePixelRatio || this.canvas.height !== i.rows * this.metrics.height * this.devicePixelRatio) && (this.resize(i.cols, i.rows), B = !0), g !== this.lastViewportY && (B = !0, this.lastViewportY = g);
    const t = D.x !== this.lastCursorPosition.x || D.y !== this.lastCursorPosition.y;
    if (t || this.cursorBlink) {
      if (!B && !A.isRowDirty(D.y)) {
        const e = A.getLine(D.y);
        e && this.renderLine(e, D.y, i.cols);
      }
      if (t && this.lastCursorPosition.y !== D.y && !B && !A.isRowDirty(this.lastCursorPosition.y)) {
        const e = A.getLine(this.lastCursorPosition.y);
        e && this.renderLine(e, this.lastCursorPosition.y, i.cols);
      }
    }
    const a = this.selectionManager && this.selectionManager.hasSelection(), N = /* @__PURE__ */ new Set();
    if (this.currentSelectionCoords = a ? this.selectionManager.getSelectionCoords() : null, this.currentSelectionCoords) {
    ...
    const w = /* @__PURE__ */ new Set(), M = this.hoveredHyperlinkId !== this.previousHoveredHyperlinkId, k = JSON.stringify(this.hoveredLinkRange) !== JSON.stringify(this.previousHoveredLinkRange);
    if (M) {
      for (let e = 0; e < i.rows; e++) {
        let G = null;
        if (g > 0)
          if (e < g && E) {
            const U = s - Math.floor(g) + e;
            G = E.getScrollbackLine(U);
          } else {
            const U = e - Math.floor(g);
            G = A.getLine(U);
          }
        else
          G = A.getLine(e);
        if (G) {
          for (const U of G)
            if (U.hyperlink_id === this.hoveredHyperlinkId || U.hyperlink_id === this.previousHoveredHyperlinkId) {
              w.add(e);
              break;
            }
        }
    ...
    for (let e = 0; e < i.rows; e++)
      (g > 0 || B || A.isRowDirty(e) || N.has(e) || w.has(e)) && (q.add(e), e > 0 && q.add(e - 1), e < i.rows - 1 && q.add(e + 1));
    for (let e = 0; e < i.rows; e++) {
      if (!q.has(e))
        continue;
      let G = null;
      if (g > 0)
        if (e < g && E) {
          ...
          const U = g > 0 ? e - Math.floor(g) : e;
          G = A.getLine(U);
        }
      else
        G = A.getLine(e);
      G && this.renderLine(G, e, i.cols);
    }
    g === 0 && D.visible && this.cursorVisible && this.renderCursor(D.x, D.y), E && C > 0 && this.renderScrollbar(g, s, i.rows, C), this.lastCursorPosition = { x: D.x, y: D.y }, A.clearDirty();
  }
  /**
  ...
  renderCellBackground(A, B, g) {
    ...
    let D = A.bg_r, i = A.bg_g, s = A.bg_b;
    A.flags & L.INVERSE && (D = A.fg_r, i = A.fg_g, s = A.fg_b), D === 0 && i === 0 && s === 0 || (this.ctx.fillStyle = this.rgbToCSS(D, i, s), this.ctx.fillRect(E, C, I, this.metrics.height));
  }
  /**
  ...
  renderCellText(A, B, g) {
    var E;
    const C = B * this.metrics.width, I = g * this.metrics.height, D = this.metrics.width * A.width;
    if (A.flags & L.INVISIBLE)
      return;
    const i = this.isInSelection(B, g);
    let s = "";
    if (A.flags & L.ITALIC && (s += "italic "), A.flags & L.BOLD && (s += "bold "), this.ctx.font = `${s}${this.fontSize}px ${this.fontFamily}`, i)
      this.ctx.fillStyle = this.theme.selectionForeground;
    else {
      let w = A.fg_r, M = A.fg_g, k = A.fg_b;
      A.flags & L.INVERSE && (w = A.bg_r, M = A.bg_g, k = A.bg_b), this.ctx.fillStyle = this.rgbToCSS(w, M, k);
    }
    A.flags & L.FAINT && (this.ctx.globalAlpha = 0.5);
    const t = C, a = I + this.metrics.baseline;
    let N;
    if (A.grapheme_len > 0 && (E = this.currentBuffer) != null && E.getGraphemeString ? N = this.currentBuffer.getGraphemeString(g, B) : N = String.fromCodePoint(A.codepoint || 32), this.ctx.fillText(N, t, a), A.flags & L.FAINT && (this.ctx.globalAlpha = 1), A.flags & L.UNDERLINE) {
      const w = I + this.metrics.baseline + 2;
      this.ctx.strokeStyle = this.ctx.fillStyle, this.ctx.lineWidth = 1, this.ctx.beginPath(), this.ctx.moveTo(C, w), this.ctx.lineTo(C + D, w), this.ctx.stroke();
    }
    if (A.flags & L.STRIKETHROUGH) {
      const w = I + this.metrics.height / 2;
      this.ctx.strokeStyle = this.ctx.fillStyle, this.ctx.lineWidth = 1, this.ctx.beginPath(), this.ctx.moveTo(C, w), this.ctx.lineTo(C + D, w), this.ctx.stroke();
    }
    if (A.hyperlink_id > 0 && A.hyperlink_id === this.hoveredHyperlinkId) {
      const w = I + this.metrics.baseline + 2;
      this.ctx.strokeStyle = "#4A90E2", this.ctx.lineWidth = 1, this.ctx.beginPath(), this.ctx.moveTo(C, w), this.ctx.lineTo(C + D, w), this.ctx.stroke();
    }
    if (this.hoveredLinkRange) {
      const w = this.hoveredLinkRange;
      if (g === w.startY && B >= w.startX && (g < w.endY || B <= w.endX) || g > w.startY && g < w.endY || g === w.endY && B <= w.endX && (g > w.startY || B >= w.startX)) {
  ...
  renderScrollbar(A, B, g, E = 1) {
    const C = this.ctx, I = this.canvas.height / this.devicePixelRatio, D = this.canvas.width / this.devicePixelRatio, i = 8, s = D - i - 4, t = 4, a = I - t * 2;
    if (C.fillStyle = this.theme.background, C.fillRect(s - 2, 0, i + 6, I), E <= 0 || B === 0)
      return;
    const N = B + g, w = Math.max(20, g / N * a), M = A / B, k = t + (a - w) * (1 - M);
    C.fillStyle = `rgba(128, 128, 128, ${0.1 * E})`, C.fillRect(s, t, i, a);
    const q = A > 0 ? 0.5 : 0.3;
    C.fillStyle = `rgba(128, 128, 128, ${q * E})`, C.fillRect(s, k, i, w);
  }
  getMetrics() {
    return { ...this.metrics };
  }
...
const DA = class CA {
  ...
  constructor(A, B, g, E) {
    this.selectionStart = null, this.selectionEnd = null, this.isSelecting = !1, this.mouseDownTarget = null, this.dirtySelectionRows = /* @__PURE__ */ new Set(), this.selectionChangedEmitter = new m(), this.boundMouseUpHandler = null, this.boundContextMenuHandler = null, this.boundClickHandler = null, this.boundDocumentMouseMoveHandler = null, this.autoScrollInterval = null, this.autoScrollDirection = 0, this.terminal = A, this.renderer = B, this.wasmTerm = g, this.textarea = E, this.attachEventListeners();
  }
  ...
  getSelection() {
    ...
    for (let D = B; D <= E; D++) {
      ...
      if (D < C)
        ...
        const w = D - C;
        i = this.wasmTerm.getLine(w);
      }
      if (!i)
        continue;
      let s = -1;
      const t = D === B ? A : 0, a = D === E ? g : i.length - 1;
      let N = "";
      for (let w = t; w <= a; w++) {
        const M = i[w];
        if (M && M.codepoint !== 0) {
          ...
          if (M.grapheme_len > 0)
            if (D < C)
              k = this.wasmTerm.getScrollbackGraphemeString(D, w);
            else {
              const q = D - C;
              k = this.wasmTerm.getGraphemeString(q, w);
            }
          else
            k = String.fromCodePoint(M.codepoint);
          N += k, k.trim() && (s = N.length);
        } else
          N += " ";
      }
      s >= 0 ? N = N.substring(0, s) : N = "", I += N, D < E && (I += `
`);
    }
    return I;
  }
    ...
    A.addEventListener("mousedown", (B) => {
      ...
      if (this.isSelecting) {
        ...
        if ((B.clientX < g.left || B.clientX > g.right || B.clientY < g.top || B.clientY > g.bottom) && (B.clientY < g.top ? this.startAutoScroll(-1) : B.clientY > g.bottom ? this.startAutoScroll(1) : this.stopAutoScroll(), this.autoScrollDirection === 0)) {
          this.markCurrentSelectionDirty();
          const i = this.pixelToCell(I, D), s = this.viewportRowToAbsolute(i.row);
          this.selectionEnd = { col: i.col, absoluteRow: s }, this.requestRender();
        }
      }
  ...
  updateAutoScroll(A, B) {
    const g = CA.AUTO_SCROLL_EDGE_SIZE;
  ...
  startAutoScroll(A) {
    this.autoScrollInterval !== null && this.autoScrollDirection === A || (this.stopAutoScroll(), this.autoScrollDirection = A, this.autoScrollInterval = setInterval(() => {
      ...
      const B = CA.AUTO_SCROLL_SPEED * this.autoScrollDirection;
...
DA.AUTO_SCROLL_EDGE_SIZE = 30;
DA.AUTO_SCROLL_SPEED = 3;
DA.AUTO_SCROLL_INTERVAL = 50;
let vA = DA;
class PA {
  ...
  constructor(A = {}) {
    this.unicode = {
      ...
      if (!this.canvas || !this.renderer || !this.linkDetector || !this.wasmTerm)
        return;
      const E = this.canvas.getBoundingClientRect(), C = Math.floor((g.clientX - E.left) / this.renderer.charWidth), I = Math.floor((g.clientY - E.top) / this.renderer.charHeight), D = this.wasmTerm.getScrollbackLength();
      let i;
      ...
      if (t > 0)
        ...
        i = D + I;
      const a = await this.linkDetector.getLinkAt(C, i);
      ...
      if (g.preventDefault(), g.stopPropagation(), !(this.customWheelEventHandler && this.customWheelEventHandler(g)))
        if (((E = this.wasmTerm) == null ? void 0 : E.isAlternateScreen()) ?? !1) {
          const D = g.deltaY > 0 ? "down" : "up", i = Math.min(Math.abs(Math.round(g.deltaY / 33)), 5);
          for (let s = 0; s < i; s++)
            D === "up" ? this.dataEmitter.fire("\x1B[A") : this.dataEmitter.fire("\x1B[B");
        } else {
          let D;
          if (g.deltaMode === WheelEvent.DOM_DELTA_PIXEL) {
            const i = ((I = (C = this.renderer) == null ? void 0 : C.getMetrics()) == null ? void 0 : I.height) ?? 20;
            D = g.deltaY / i;
          } else
            g.deltaMode === WheelEvent.DOM_DELTA_LINE ? D = g.deltaY : g.deltaMode === WheelEvent.DOM_DELTA_PAGE ? D = g.deltaY * this.rows : D = g.deltaY / 33;
          if (D !== 0) {
      ...
      if (E === 0)
        return;
      const C = this.canvas.getBoundingClientRect(), I = g.clientX - C.left, D = g.clientY - C.top, i = C.width, s = C.height, t = 8, a = i - t - 4, N = 4;
      if (I >= a && I <= a + t) {
        g.preventDefault(), g.stopPropagation(), g.stopImmediatePropagation();
        const w = s - N * 2, M = this.rows, k = E + M, q = Math.max(20, M / k * w), e = this.viewportY / E, G = N + (w - q) * (1 - e);
        if (D >= G && D <= G + q)
          ...
          const U = 1 - (D - N) / w, n = Math.round(U * E);
    ...
    }), this.cols = this.options.cols, this.rows = this.options.rows, this.buffer = new bA(this);
  }
  // ==========================================================================
  ...
  open(A) {
    ...
    try {
      ...
      this.canvas.addEventListener("mousedown", (E) => {
        ...
        E.preventDefault(), g.focus();
      }), this.renderer = new jA(this.canvas, {
        fontSize: this.options.fontSize,
        fontFamily: this.options.fontFamily,
        cursorStyle: this.options.cursorStyle,
        cursorBlink: this.options.cursorBlink,
        theme: this.options.theme
      }), this.renderer.resize(this.cols, this.rows), this.inputHandler = new zA(
        this.ghostty,
        A,
        (E) => {
...
`, B);
  ...
  processMouseMove(A) {
    if (!this.canvas || !this.renderer || !this.linkDetector || !this.wasmTerm)
      return;
    const B = this.canvas.getBoundingClientRect(), g = Math.floor((A.clientX - B.left) / this.renderer.charWidth), E = Math.floor((A.clientY - B.top) / this.renderer.charHeight);
    let C = 0, I = null;
    const D = this.getViewportY(), i = Math.max(0, Math.floor(D));
    if (i > 0) {
      const M = this.wasmTerm.getScrollbackLength();
      if (E < i) {
        const k = M - i + E;
        I = this.wasmTerm.getScrollbackLine(k);
      } else {
        const k = E - i;
        I = this.wasmTerm.getLine(k);
      }
    } else
    ...
    let a;
    const N = this.getViewportY(), w = Math.max(0, Math.floor(N));
    if (w > 0)
      if (E < w)
        a = t - w + E;
      else {
      ...
      a = t + E;
    this.linkDetector.getLinkAt(g, a).then((M) => {
      var k, q, e, G;
      if (M !== this.currentHoveredLink && ((q = (k = this.currentHoveredLink) == null ? void 0 : k.hover) == null || q.call(k, !1), this.currentHoveredLink = M, (e = M == null ? void 0 : M.hover) == null || e.call(M, !0), this.element && (this.element.style.cursor = M ? "pointer" : "text"), this.renderer))
        if (M) {
          const U = ((G = this.wasmTerm) == null ? void 0 : G.getScrollbackLength()) || 0, n = this.getViewportY(), b = Math.max(0, Math.floor(n)), O = M.range.start.y - U + b, d = M.range.end.y - U + b;
          O < this.rows && d >= 0 ? this.renderer.setHoveredLinkRange({
            startX: M.range.start.x,
            startY: Math.max(0, O),
            endX: M.range.end.x,
      ...
      console.warn("Link detection error:", M);
    });
  }
  /**
  ...
  processScrollbarDrag(A) {
    ...
    if (B === 0)
      return;
    const g = this.canvas.getBoundingClientRect(), E = A.clientY - g.top - this.scrollbarDragStart, C = g.height - 8, I = this.rows, D = B + I, i = Math.max(20, I / D * C), s = -E / (C - i), t = Math.round(s * B), a = this.scrollbarDragStartViewportY + t;
...
class QQ {
  ...
  proposeDimensions() {
    ...
    if (typeof E.clientWidth > "u")
      return;
    const C = window.getComputedStyle(E), I = Number.parseInt(C.getPropertyValue("padding-top")) || 0, D = Number.parseInt(C.getPropertyValue("padding-bottom")) || 0, i = Number.parseInt(C.getPropertyValue("padding-left")) || 0, s = Number.parseInt(C.getPropertyValue("padding-right")) || 0, t = E.clientWidth, a = E.clientHeight;
    if (t === 0 || a === 0)
      return;
    const N = t - i - s - $A, w = a - I - D, M = Math.max(XA, Math.floor(N / g.width)), k = Math.max(_A, Math.floor(w / g.height));
...
function EQ(Q) {
  return Array.isArray(Q) && // must be at least one error
  Q.length > 0 && // error has at least a message
  Q.every((A) => "message" in A);
}
function GA(Q, A) {
  return Q.length < 124 ? Q : A;
}
const CQ = "graphql-transport-ws";
var T;
(function(Q) {
  Q[Q.InternalServerError = 4500] = "InternalServerError", Q[Q.InternalClientError = 4005] = "InternalClientError", Q[Q.BadRequest = 4400] = "BadRequest", Q[Q.BadResponse = 4004] = "BadResponse", Q[Q.Unauthorized = 4401] = "Unauthorized", Q[Q.Forbidden = 4403] = "Forbidden", Q[Q.SubprotocolNotAcceptable = 4406] = "SubprotocolNotAcceptable", Q[Q.ConnectionInitialisationTimeout = 4408] = "ConnectionInitialisationTimeout", Q[Q.ConnectionAcknowledgementTimeout = 4504] = "ConnectionAcknowledgementTimeout", Q[Q.SubscriberAlreadyExists = 4409] = "SubscriberAlreadyExists", Q[Q.TooManyInitialisationRequests = 4429] = "TooManyInitialisationRequests";
})(T || (T = {}));
var R;
(function(Q) {
  Q.ConnectionInit = "connection_init", Q.ConnectionAck = "connection_ack", Q.Ping = "ping", Q.Pong = "pong", Q.Subscribe = "subscribe", Q.Next = "next", Q.Error = "error", Q.Complete = "complete";
})(R || (R = {}));
function UA(Q) {
  if (!Z(Q))
    throw new Error(`Message is expected to be an object, but got ${x(Q)}`);
  if (!Q.type)
    throw new Error("Message is missing the 'type' property");
  if (typeof Q.type != "string")
    throw new Error(`Message is expects the 'type' property to be a string, but got ${x(Q.type)}`);
  switch (Q.type) {
    case R.ConnectionInit:
    case R.ConnectionAck:
    case R.Ping:
    case R.Pong: {
      if (Q.payload != null && !Z(Q.payload))
        throw new Error(`"${Q.type}" message expects the 'payload' property to be an object or nullish or missing, but got "${Q.payload}"`);
      break;
    }
    case R.Subscribe: {
      if (typeof Q.id != "string")
        throw new Error(`"${Q.type}" message expects the 'id' property to be a string, but got ${x(Q.id)}`);
      if (!Q.id)
        throw new Error(`"${Q.type}" message requires a non-empty 'id' property`);
      if (!Z(Q.payload))
        throw new Error(`"${Q.type}" message expects the 'payload' property to be an object, but got ${x(Q.payload)}`);
      if (typeof Q.payload.query != "string")
        throw new Error(`"${Q.type}" message payload expects the 'query' property to be a string, but got ${x(Q.payload.query)}`);
      if (Q.payload.variables != null && !Z(Q.payload.variables))
        throw new Error(`"${Q.type}" message payload expects the 'variables' property to be a an object or nullish or missing, but got ${x(Q.payload.variables)}`);
      if (Q.payload.operationName != null && x(Q.payload.operationName) !== "string")
        throw new Error(`"${Q.type}" message payload expects the 'operationName' property to be a string or nullish or missing, but got ${x(Q.payload.operationName)}`);
      if (Q.payload.extensions != null && !Z(Q.payload.extensions))
        throw new Error(`"${Q.type}" message payload expects the 'extensions' property to be a an object or nullish or missing, but got ${x(Q.payload.extensions)}`);
      break;
    }
    case R.Next: {
      if (typeof Q.id != "string")
    ...
    case R.Error: {
      ...
      if (!Q.id)
        throw new Error(`"${Q.type}" message requires a non-empty 'id' property`);
      if (!EQ(Q.payload))
        throw new Error(`"${Q.type}" message expects the 'payload' property to be an array of GraphQL errors, but got ${JSON.stringify(Q.payload)}`);
      break;
    }
    case R.Complete: {
      if (typeof Q.id != "string")
    ...
    default:
      throw new Error(`Invalid message 'type' property "${Q.type}"`);
  }
  return Q;
}
function IQ(Q, A) {
  return UA(typeof Q == "string" ? JSON.parse(Q, A) : Q);
}
function $(Q, A) {
  return UA(Q), JSON.stringify(Q, A);
}
var v = function(Q) {
  return this instanceof v ? (this.v = Q, this) : new v(Q);
}, DQ = function(Q, A, B) {
  if (!Symbol.asyncIterator) throw new TypeError("Symbol.asyncIterator is not defined.");
  var g = B.apply(Q, A || []), E, C = [];
  return E = Object.create((typeof AsyncIterator == "function" ? AsyncIterator : Object).prototype), D("next"), D("throw"), D("return", I), E[Symbol.asyncIterator] = function() {
    return this;
  }, E;
  function I(w) {
    return function(M) {
  ...
  function s(w) {
    w.value instanceof v ? Promise.resolve(w.value.v).then(t, a) : N(C[0][2], w);
  }
  function t(w) {
    i("next", w);
  }
  ...
  function N(w, M) {
    w(M), C.shift(), C.length && i(C[0][0], C[0][1]);
  }
};
function oQ(Q) {
  const {
    ...
    connectionParams: B,
    lazy: g = !0,
    onNonLazyError: E = console.error,
    lazyCloseTimeout: C = 0,
    keepAlive: I = 0,
    disablePong: D,
    connectionAckWaitTimeout: i = 0,
    retryAttempts: s = 5,
    retryWait: t = async function(r) {
      let c = 1e3;
      for (let J = 0; J < r; J++)
        c *= 2;
      await new Promise((J) => setTimeout(J, c + // add random timeout from 300ms to 3s
    ...
    generateID: k = function() {
      return "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g, (r) => {
        ...
        return (r == "x" ? c : c & 3 | 8).toString(16);
      });
    },
    jsonMessageReplacer: q,
  ...
  if (M) {
    ...
    G = M;
  } else typeof WebSocket < "u" ? G = WebSocket : typeof global < "u" ? G = global.WebSocket || // @ts-expect-error: Support more browsers
  ...
  const U = G, n = (() => {
    const F = /* @__PURE__ */ (() => {
      ...
      return {
        ...
        emit(J) {
          var y;
          "id" in J && ((y = c[J.id]) === null || y === void 0 || y.call(c, J));
        }
      };
    })(), r = {
      connecting: w != null && w.connecting ? [w.connecting] : [],
      opened: w != null && w.opened ? [w.opened] : [],
      connected: w != null && w.connected ? [w.connected] : [],
      ping: w != null && w.ping ? [w.ping] : [],
      pong: w != null && w.pong ? [w.pong] : [],
      message: w != null && w.message ? [F.emit, w.message] : [F.emit],
      closed: w != null && w.closed ? [w.closed] : [],
      error: w != null && w.error ? [w.error] : []
    };
    return {
      onMessage: F.on,
      on(c, J) {
  ...
  let O, d = 0, BA, z = !1, h = 0, Y = !1;
  async function H() {
    clearTimeout(BA);
    const [F, r] = await (O ?? (O = new Promise((y, p) => (async () => {
      if (z) {
        if (await t(h), !d)
          return O = void 0, p({ code: 1e3, reason: "All Subscriptions Gone" });
        h++;
      }
      n.emit("connecting", z);
      const K = new U(typeof A == "function" ? await A() : A, CQ);
      let V, X;
      function gA() {
        isFinite(I) && I > 0 && (clearTimeout(X), X = setTimeout(() => {
          K.readyState === U.OPEN && (K.send($({ type: R.Ping })), n.emit("ping", !1, void 0));
        }, I));
      }
      b((S) => {
        O = void 0, clearTimeout(V), clearTimeout(X), p(S), S instanceof FA && (K.close(4499, "Terminated"), K.onerror = null, K.onclose = null);
      }), K.onerror = (S) => n.emit("error", S), K.onclose = (S) => n.emit("closed", S), K.onopen = async () => {
        try {
          n.emit("opened", K);
          const S = typeof B == "function" ? await B() : B;
          if (K.readyState !== U.OPEN)
            return;
          K.send($(S ? {
            type: R.ConnectionInit,
            payload: S
          } : {
            type: R.ConnectionInit
            // payload is completely absent if not provided
          }, q)), isFinite(i) && i > 0 && (V = setTimeout(() => {
            K.close(T.ConnectionAcknowledgementTimeout, "Connection acknowledgement timeout");
          }, i)), gA();
        } catch (S) {
          n.emit("error", S), K.close(T.InternalClientError, GA(S instanceof Error ? S.message : new Error(S).message, "Internal client error"));
        }
      };
      let j = !1;
      K.onmessage = ({ data: S }) => {
        try {
          const l = IQ(S, e);
          if (n.emit("message", l), l.type === "ping" || l.type === "pong") {
            n.emit(l.type, !0, l.payload), l.type === "pong" ? gA() : D || (K.send($(l.payload ? {
              type: R.Pong,
              payload: l.payload
            } : {
              type: R.Pong
              // payload is completely absent if not provided
            })), n.emit("pong", !1, l.payload));
            return;
          }
          if (j)
            return;
          if (l.type !== R.ConnectionAck)
            throw new Error(`First message cannot be of type ${l.type}`);
          clearTimeout(V), j = !0, n.emit("connected", K, l.payload, z), z = !1, h = 0, y([
            K,
            new Promise((KQ, rA) => b(rA))
          ]);
        } catch (l) {
          K.onmessage = null, n.emit("error", l), K.close(T.BadResponse, GA(l instanceof Error ? l.message : new Error(l).message, "Bad response"));
        }
      };
    })())));
    F.readyState === U.CLOSING && await r;
    let c = () => {
    };
    const J = new Promise((y) => c = y);
    return [
      F,
      c,
      Promise.race([
        ...
        J.then(() => {
          if (!d) {
            const y = () => F.close(1e3, "Normal Closure");
            isFinite(C) && C > 0 ? BA = setTimeout(() => {
              F.readyState === U.OPEN && y();
            }, C) : y();
          }
        }),
        // or
  ...
  function P(F) {
    if (iA(F) && (iQ(F.code) || [
      ...
      T.InternalClientError,
      T.BadRequest,
      T.BadResponse,
      T.Unauthorized,
      // CloseCode.Forbidden, might grant access out after retry
      T.SubprotocolNotAcceptable,
      // CloseCode.ConnectionInitialisationTimeout, might not time out after retry
      // CloseCode.ConnectionAcknowledgementTimeout, might not time out after retry
      T.SubscriberAlreadyExists,
      T.TooManyInitialisationRequests
    ...
    iterate(F) {
      const r = [], c = {
        ...
        return DQ(this, arguments, function* () {
          for (; ; ) {
            for (r.length || (yield v(new Promise((V) => c.resolve = V))); r.length; )
              yield yield v(r.shift());
            if (c.error)
              throw c.error;
            if (c.done)
      ...
      return y.throw = async (p) => (c.done || (c.done = !0, c.error = p, c.resolve()), { done: !0, value: void 0 }), y.return = async () => (J(), { done: !0, value: void 0 }), y;
    },
    async dispose() {
      if (Y = !0, O) {
        const [F] = await O;
        F.close(1e3, "Normal Closure");
      }
    },
    terminate() {
      O && n.emit("closed", new FA());
    }
  };
}
...
function iA(Q) {
  return Z(Q) && "code" in Q && "reason" in Q;
}
function iQ(Q) {
  return [
    1e3,
    ...
    1014
    // Bad Gateway
  ].includes(Q) ? !1 : Q >= 1e3 && Q <= 1999;
}
function wQ(Q) {
  return typeof Q == "function" && "constructor" in Q && "CLOSED" in Q && "CLOSING" in Q && "CONNECTING" in Q && "OPEN" in Q;
}
function sQ() {
...
function tQ(Q, A, B, g) {
  const E = oQ({
    ...
    { query: Q, variables: A },
    {
      next: (I) => {
        var D, i;
        if ((D = I.errors) != null && D.length) {
          g(new Error(((i = I.errors[0]) == null ? void 0 : i.message) ?? "Subscription error"));
          return;
        }
        I.data && B(I.data);
      },
...
const MQ = { class: "fixed inset-0 z-50 flex items-center justify-center bg-black/70" }, aQ = {
  class: "flex shrink-0 items-center gap-2.5",
  style: { height: "42px", padding: "0 14px", "border-bottom": "1px solid hsl(var(--border))", background: "hsl(var(--card))" }
}, eQ = {
  id: "terminal-dialog-title",
  class: "min-w-0 flex-1 truncate text-muted-foreground font-mono text-xs font-medium"
}, NQ = {
  class: "ml-auto flex items-center gap-1.5 text-muted-foreground text-xs",
  "aria-live": "polite"
}, hQ = {
  key: 0,
  role: "alert",
  class: "border-b border-destructive/40 bg-destructive/10 px-4 py-2 text-sm text-destructive"
}, kQ = { class: "bg-background p-4" }, cQ = `
...
`, JA = "mutation($sessionId: String!) { stopJailExec(sessionId: $sessionId) }", JQ = "subscription($sessionId: String!) { jailExecOutput(sessionId: $sessionId) }", UQ = /* @__PURE__ */ qA({
  __name: "Terminal",
  props: {
  ...
  emits: ["close"],
  setup(Q, { emit: A }) {
    const B = Q, g = A, E = EA(null), C = EA(null), I = EA(null), D = EA("connecting");
    let i = !1, s = null, t = null, a = null, N = null, w = null, M = null, k = !0, q = !1, e = null;
    function G(h, Y) {
      const H = getComputedStyle(document.documentElement).getPropertyValue(h).trim();
      return H ? /^\d+(?:\.\d+)?\s+\d+(?:\.\d+)?%\s+\d+(?:\.\d+)?%$/.test(H) ? `hsl(${H})` : H : Y;
    }
    function U() {
      const h = G("--background", "#111111"), Y = G("--foreground", "#f5f5f5"), H = G("--primary", "#ff8c2f");
      return {
        background: h,
        foreground: Y,
        cursor: H,
        selectionBackground: `color-mix(in srgb, ${H} 22%, transparent)`,
        black: h,
        red: G("--destructive", "#c0392b"),
        green: G("--color-unraid-green", "#63a659"),
        yellow: G("--color-yellow-accent", "#e9bf41"),
        blue: G("--color-primary-500", "#ff8c2f"),
        magenta: "#c084fc",
        cyan: H,
        white: Y,
        brightBlack: G("--muted-foreground", "#999999"),
        brightRed: "#ef6b6b",
        brightGreen: "#7db474",
        brightYellow: "#f0cf6a",
        brightBlue: "#fb923c",
    ...
    async function n() {
      if (E.value && (await BQ(), !(!k || !E.value))) {
        s = new PA({
          ...
          theme: U()
        }), t = new QQ(), s.loadAddon(t), s.open(E.value), t.fit(), t.observeResize();
        try {
          if (a = (await _(cQ, {
            name: B.jailName,
            cols: s.cols,
            ...
            const Y = a;
            a = null, await _(JA, { sessionId: Y }).catch(() => {
            }), d();
            return;
          }
        } catch (h) {
          if (!k) return;
          I.value = h instanceof Error ? h.message : String(h), D.value = "disconnected";
          return;
        }
        D.value = "connected", N = tQ(
          JQ,
          { sessionId: a },
          (h) => s == null ? void 0 : s.write(h.jailExecOutput),
          (h) => {
            I.value = h instanceof Error ? h.message : String(h), D.value = "disconnected";
          }
        ), w = s.onData((h) => {
          a && _(GQ, { sessionId: a, data: h }).catch(b);
        }), M = s.onResize(({ cols: h, rows: Y }) => {
          a && _(FQ, { sessionId: a, cols: h, rows: Y }).catch(b);
        });
      }
    }
    function b(h) {
      i || (i = !0, I.value = h instanceof Error ? h.message : String(h), D.value = "disconnected");
    }
    async function O() {
      if (k = !1, q) return;
      q = !0;
      const h = a;
      a = null, d(), h && await _(JA, { sessionId: h }).catch(b);
    }
    function d() {
      w == null || w.dispose(), M == null || M.dispose(), t == null || t.dispose(), N == null || N(), s == null || s.dispose(), w = null, M = null, t = null, N = null, s = null;
    }
    function BA(h) {
      if (h.key === "Escape") {
        h.preventDefault(), z();
      ...
      if (h.key !== "Tab" || !C.value) return;
      const Y = Array.from(C.value.querySelectorAll(
        'button:not([disabled]), [href], input:not([disabled]), [tabindex]:not([tabindex="-1"])'
      ));
      if (Y.length === 0) {
        h.preventDefault(), C.value.focus();
        return;
      }
      const H = Y[0], P = Y.at(-1);
      h.shiftKey && document.activeElement === H ? (h.preventDefault(), P.focus()) : !h.shiftKey && document.activeElement === P && (h.preventDefault(), H.focus());
    }
    nA(async () => {
      var h;
      k = !0, e = document.activeElement instanceof HTMLElement ? document.activeElement : null, await YA(), (h = C.value) == null || h.focus(), n().catch(b);
    }), RA(() => {
      O(), e == null || e.focus();
    });
    function z() {
    ...
    return (h, Y) => (eA(), NA("div", MQ, [
      f("div", {
        ...
        style: { border: "1px solid hsl(var(--border))", "border-radius": "var(--radius)", background: "hsl(var(--background))", "box-shadow": "0 20px 38px rgba(0, 0, 0, 0.26)" }
      }, [
        f("div", aQ, [
          Y[1] || (Y[1] = LA('<svg width="10" height="14" viewBox="0 0 10 14" fill="none" aria-hidden="true" class="shrink-0 text-primary"><path d="M5 0L9 2.5L5 5L1 2.5Z" fill="currentColor" opacity="0.95"></path><path d="M5 3L9 5.5L5 8L1 5.5Z" fill="currentColor" opacity="0.75"></path><path d="M5 6L9 8.5L5 11L1 8.5Z" fill="currentColor" opacity="0.5"></path><path d="M5 9L9 11.5L5 14L1 11.5Z" fill="currentColor" opacity="0.28"></path></svg>', 1)),
          f("span", eQ, oA(Q.jailName), 1),
          Y[2] || (Y[2] = f("span", { style: { padding: "3px 7px", border: "1px solid hsl(var(--primary) / 0.35)", "border-radius": "4px", background: "hsl(var(--primary) / 0.12)", color: "hsl(var(--foreground))", "font-family": "ui-monospace, monospace", "font-size": "10.5px", "font-weight": "600" } }, "ghostty-web", -1)),
          f("div", NQ, [
            f("span", {
              style: lA([{ width: "7px", height: "7px", "border-radius": "999px" }, {
                background: D.value === "connected" ? "var(--color-unraid-green, #63a659)" : D.value === "connecting" ? "var(--color-yellow-accent, #e9bf41)" : "hsl(var(--destructive))"
              }])
            }, null, 4),
            f("span", null, oA(D.value === "connected" ? "Connected" : D.value === "connecting" ? "Connecting…" : "Disconnected"), 1)
          ]),
          SA(pA(xA), {
            size: "sm",
            variant: "outline",
        ...
        I.value ? (eA(), NA("div", hQ, oA(I.value), 1)) : dA("", !0),
        f("div", kQ, [
          f("div", {
            ref_key: "containerEl",
            ref: E,
            class: "h-[60vh] w-full overflow-hidden"
          }, null, 512)
        ])
      ], 544)
    ]));
