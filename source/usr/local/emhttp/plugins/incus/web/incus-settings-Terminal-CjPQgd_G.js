...
class pA {
  ...
  setKittyFlags(A) {
    this.setOption(wA.KITTY_KEYBOARD_FLAGS, A);
  }
  encode(A) {
    ...
    const I = 32, D = this.exports.ghostty_wasm_alloc_u8_array(I), o = this.exports.ghostty_wasm_alloc_usize(), s = this.exports.ghostty_key_encoder_encode(
      this.encoder,
      C,
      D,
      I,
    ...
    if (s !== 0)
      throw this.exports.ghostty_wasm_free_u8_array(D, I), this.exports.ghostty_wasm_free_usize(o), this.exports.ghostty_key_event_free(C), new Error(`Failed to encode key: ${s}`);
    const t = E.getUint32(o, !0), e = new Uint8Array(this.exports.memory.buffer, D, t).slice();
    return this.exports.ghostty_wasm_free_u8_array(D, I), this.exports.ghostty_wasm_free_usize(o), this.exports.ghostty_key_event_free(C), e;
  }
  dispose() {
...
const KA = class m {
  constructor(A, B, g = 80, E = 24, C) {
    ...
    if (this.viewportBufferPtr = 0, this.viewportBufferSize = 0, this.cellPool = [], this.graphemeBuffer = null, this.graphemeBufferPtr = 0, this.exports = A, this.memory = B, this._cols = g, this._rows = E, C) {
      ...
      try {
        ...
        let s = D;
        o.setUint32(s, C.scrollbackLimit ?? 1e4, !0), s += 4, o.setUint32(s, C.fgColor ?? 0, !0), s += 4, o.setUint32(s, C.bgColor ?? 0, !0), s += 4, o.setUint32(s, C.cursorColor ?? 0, !0), s += 4;
        for (let t = 0; t < 16; t++)
          o.setUint32(s, ((I = C.palette) == null ? void 0 : I[t]) ?? 0, !0), s += 4;
   ...
   */
  getViewport() {
    const A = this._cols * this._rows, B = A * m.CELL_SIZE;
   ...
   */
  getScrollbackLine(A) {
    const B = this._cols * m.CELL_SIZE;
    ...
    if (g < 0)
      return null;
    const E = [], C = this.memory.buffer, I = new Uint8Array(C, this.viewportBufferPtr, g * m.CELL_SIZE), D = new DataView(C, this.viewportBufferPtr, g * m.CELL_SIZE);
    for (let o = 0; o < g; o++) {
      const s = o * m.CELL_SIZE;
      E.push({
  ...
  parseCellsIntoPool(A, B) {
    const g = this.memory.buffer, E = new Uint8Array(g, A, B * m.CELL_SIZE), C = new DataView(g, A, B * m.CELL_SIZE);
    for (let I = 0; I < B; I++) {
      const D = I * m.CELL_SIZE, o = this.cellPool[I];
...
class dA {
  constructor(A) {
    this.bufferChangeEmitter = new x(), this.terminal = A;
  }
  get active() {
...
class TA {
  ...
  getCell(A) {
    if (!(A < 0 || A >= this._length))
      return A >= this.cells.length ? new iA(
        {
          codepoint: 0,
          fg_r: 204,
          fg_g: 204,
...
class iA {
  ...
  isBold() {
    return this.cell.flags & Y.BOLD ? 1 : 0;
  }
  isItalic() {
    return this.cell.flags & Y.ITALIC ? 1 : 0;
  }
  isUnderline() {
    return this.cell.flags & Y.UNDERLINE ? 1 : 0;
  }
  isStrikethrough() {
    return this.cell.flags & Y.STRIKETHROUGH ? 1 : 0;
  }
  isBlink() {
    return this.cell.flags & Y.BLINK ? 1 : 0;
  }
  isInverse() {
    return this.cell.flags & Y.INVERSE ? 1 : 0;
  }
  isInvisible() {
    return this.cell.flags & Y.INVISIBLE ? 1 : 0;
  }
  isFaint() {
    return this.cell.flags & Y.FAINT ? 1 : 0;
  }
  /**
   ...
   */
  isDim() {
    return (this.cell.flags & Y.FAINT) !== 0;
  }
}
const mA = {
  // Letters
...
class bA {
  ...
  extractModifiers(A) {
    let B = f.NONE;
    return A.shiftKey && (B |= f.SHIFT), A.ctrlKey && (B |= f.CTRL), A.altKey && (B |= f.ALT), A.metaKey && (B |= f.SUPER), B;
  }
  /**
  ...
  handleKeyDown(A) {
    ...
    try {
      if (this.getModeCallback) {
        ...
        this.encoder.setOption(wA.CURSOR_KEY_APPLICATION, o);
      }
...
class fA {
  ...
  cacheLink(A) {
    ...
    if (g) {
      ...
      if (!D) {
        const { start: s, end: t } = A.range, e = `r${s.y}:${s.x}-${t.x}`;
  ...
  provideLinks(A, B) {
    ...
    for (let I = 0; I < C.length; I++) {
      ...
      if (o === 0 || E.has(o))
        continue;
      E.add(o);
      const s = this.findLinkRange(o, A, I);
      if (!this.terminal.wasmTerm)
        continue;
      const t = this.terminal.wasmTerm.getHyperlinkUri(o);
      t && g.push({
        text: t,
        range: s,
        activate: (e) => {
...
const sA = class v {
  ...
  provideLinks(A, B) {
    ...
    v.URL_REGEX.lastIndex = 0;
    let I = v.URL_REGEX.exec(C);
    for (; I !== null; ) {
      let D = I[0];
      const o = I.index;
      let s = I.index + D.length - 1;
      const t = D.replace(v.TRAILING_PUNCTUATION, "");
      t.length < D.length && (D = t, s = o + D.length - 1), D.length > 8 && g.push({
        text: D,
        range: {
          start: { x: o, y: A },
          end: { x: s, y: A }
        ...
        activate: (e) => {
          (e.ctrlKey || e.metaKey) && window.open(D, "_blank", "noopener,noreferrer");
        }
      }), I = v.URL_REGEX.exec(C);
    }
    B(g.length > 0 ? g : void 0);
  }
  /**
...
sA.URL_REGEX = /(?:https?:\/\/|mailto:|ftp:\/\/|ssh:\/\/|git:\/\/|tel:|magnet:|gemini:\/\/|gopher:\/\/|news:)[\w\-.~:\/?#@!$&*+,;=%]+/gi;
sA.TRAILING_PUNCTUATION = /[.,;!?)\]]+$/;
let zA = sA;
const cA = {
  foreground: "#d4d4d4",
  background: "#1e1e1e",
...
class VA {
  ...
  render(A, B = !1, g = 0, E, C = 1) {
    ...
    const D = A.getCursor(), o = A.getDimensions(), s = E ? E.getScrollbackLength() : 0;
    (I = A.needsFullRedraw) != null && I.call(A) && (B = !0), (this.canvas.width !== o.cols * this.metrics.width * this.devicePixelRatio || this.canvas.height !== o.rows * this.metrics.height * this.devicePixelRatio) && (this.resize(o.cols, o.rows), B = !0), g !== this.lastViewportY && (B = !0, this.lastViewportY = g);
    const t = D.x !== this.lastCursorPosition.x || D.y !== this.lastCursorPosition.y;
    if (t || this.cursorBlink) {
      if (!B && !A.isRowDirty(D.y)) {
        const a = A.getLine(D.y);
        a && this.renderLine(a, D.y, o.cols);
      }
    ...
    const e = this.selectionManager && this.selectionManager.hasSelection(), N = /* @__PURE__ */ new Set();
    if (this.currentSelectionCoords = e ? this.selectionManager.getSelectionCoords() : null, this.currentSelectionCoords) {
      const a = this.currentSelectionCoords;
      for (let K = a.startRow; K <= a.endRow; K++)
        N.add(K);
    }
    ...
    const w = /* @__PURE__ */ new Set(), M = this.hoveredHyperlinkId !== this.previousHoveredHyperlinkId, G = JSON.stringify(this.hoveredLinkRange) !== JSON.stringify(this.previousHoveredLinkRange);
    if (M) {
      for (let a = 0; a < o.rows; a++) {
        let K = null;
        if (g > 0)
          if (a < g && E) {
            const k = s - Math.floor(g) + a;
            K = E.getScrollbackLine(k);
          } else {
            const k = a - Math.floor(g);
            K = A.getLine(k);
          }
        else
          K = A.getLine(a);
        if (K) {
          for (const k of K)
            if (k.hyperlink_id === this.hoveredHyperlinkId || k.hyperlink_id === this.previousHoveredHyperlinkId) {
              w.add(a);
              break;
            }
        }
    ...
    for (let a = 0; a < o.rows; a++)
      (g > 0 || B || A.isRowDirty(a) || N.has(a) || w.has(a)) && (r.add(a), a > 0 && r.add(a - 1), a < o.rows - 1 && r.add(a + 1));
    for (let a = 0; a < o.rows; a++) {
      if (!r.has(a))
        continue;
      let K = null;
      if (g > 0)
        if (a < g && E) {
          ...
          const k = g > 0 ? a - Math.floor(g) : a;
          K = A.getLine(k);
        }
      else
        K = A.getLine(a);
      K && this.renderLine(K, a, o.cols);
    }
    g === 0 && D.visible && this.cursorVisible && this.renderCursor(D.x, D.y), E && C > 0 && this.renderScrollbar(g, s, o.rows, C), this.lastCursorPosition = { x: D.x, y: D.y }, A.clearDirty();
  }
  /**
  ...
  renderCellBackground(A, B, g) {
    ...
    let D = A.bg_r, o = A.bg_g, s = A.bg_b;
    A.flags & Y.INVERSE && (D = A.fg_r, o = A.fg_g, s = A.fg_b), D === 0 && o === 0 && s === 0 || (this.ctx.fillStyle = this.rgbToCSS(D, o, s), this.ctx.fillRect(E, C, I, this.metrics.height));
  }
  /**
  ...
  renderCellText(A, B, g) {
    ...
    if (A.flags & Y.INVISIBLE)
      return;
    const o = this.isInSelection(B, g);
    let s = "";
    if (A.flags & Y.ITALIC && (s += "italic "), A.flags & Y.BOLD && (s += "bold "), this.ctx.font = `${s}${this.fontSize}px ${this.fontFamily}`, o)
      this.ctx.fillStyle = this.theme.selectionForeground;
    else {
      let w = A.fg_r, M = A.fg_g, G = A.fg_b;
      A.flags & Y.INVERSE && (w = A.bg_r, M = A.bg_g, G = A.bg_b), this.ctx.fillStyle = this.rgbToCSS(w, M, G);
    }
    A.flags & Y.FAINT && (this.ctx.globalAlpha = 0.5);
    const t = C, e = I + this.metrics.baseline;
    let N;
    if (A.grapheme_len > 0 && (E = this.currentBuffer) != null && E.getGraphemeString ? N = this.currentBuffer.getGraphemeString(g, B) : N = String.fromCodePoint(A.codepoint || 32), this.ctx.fillText(N, t, e), A.flags & Y.FAINT && (this.ctx.globalAlpha = 1), A.flags & Y.UNDERLINE) {
  ...
  renderScrollbar(A, B, g, E = 1) {
    const C = this.ctx, I = this.canvas.height / this.devicePixelRatio, D = this.canvas.width / this.devicePixelRatio, o = 8, s = D - o - 4, t = 4, e = I - t * 2;
    if (C.fillStyle = this.theme.background, C.fillRect(s - 2, 0, o + 6, I), E <= 0 || B === 0)
      return;
    const N = B + g, w = Math.max(20, g / N * e), M = A / B, G = t + (e - w) * (1 - M);
    C.fillStyle = `rgba(128, 128, 128, ${0.1 * E})`, C.fillRect(s, t, o, e);
    const r = A > 0 ? 0.5 : 0.3;
    C.fillStyle = `rgba(128, 128, 128, ${r * E})`, C.fillRect(s, G, o, w);
  }
  getMetrics() {
    return { ...this.metrics };
  }
...
const BA = class AA {
  ...
  constructor(A, B, g, E) {
    this.selectionStart = null, this.selectionEnd = null, this.isSelecting = !1, this.mouseDownTarget = null, this.dirtySelectionRows = /* @__PURE__ */ new Set(), this.selectionChangedEmitter = new x(), this.boundMouseUpHandler = null, this.boundContextMenuHandler = null, this.boundClickHandler = null, this.boundDocumentMouseMoveHandler = null, this.autoScrollInterval = null, this.autoScrollDirection = 0, this.terminal = A, this.renderer = B, this.wasmTerm = g, this.textarea = E, this.attachEventListeners();
  }
  ...
  getSelection() {
    ...
    for (let D = B; D <= E; D++) {
      ...
      if (!o)
        continue;
      let s = -1;
      const t = D === B ? A : 0, e = D === E ? g : o.length - 1;
      let N = "";
      for (let w = t; w <= e; w++) {
        const M = o[w];
        if (M && M.codepoint !== 0) {
          ...
          if (M.grapheme_len > 0)
            if (D < C)
              G = this.wasmTerm.getScrollbackGraphemeString(D, w);
            else {
              const r = D - C;
              G = this.wasmTerm.getGraphemeString(r, w);
            }
          else
            G = String.fromCodePoint(M.codepoint);
          N += G, G.trim() && (s = N.length);
        } else
          N += " ";
      }
...
`);
    ...
    A.addEventListener("mousedown", (B) => {
      ...
      if (this.isSelecting) {
        ...
        if ((B.clientX < g.left || B.clientX > g.right || B.clientY < g.top || B.clientY > g.bottom) && (B.clientY < g.top ? this.startAutoScroll(-1) : B.clientY > g.bottom ? this.startAutoScroll(1) : this.stopAutoScroll(), this.autoScrollDirection === 0)) {
          this.markCurrentSelectionDirty();
          const o = this.pixelToCell(I, D), s = this.viewportRowToAbsolute(o.row);
          this.selectionEnd = { col: o.col, absoluteRow: s }, this.requestRender();
        }
      }
  ...
  updateAutoScroll(A, B) {
    const g = AA.AUTO_SCROLL_EDGE_SIZE;
  ...
  startAutoScroll(A) {
    this.autoScrollInterval !== null && this.autoScrollDirection === A || (this.stopAutoScroll(), this.autoScrollDirection = A, this.autoScrollInterval = setInterval(() => {
      ...
      const B = AA.AUTO_SCROLL_SPEED * this.autoScrollDirection;
...
BA.AUTO_SCROLL_EDGE_SIZE = 30;
BA.AUTO_SCROLL_SPEED = 3;
BA.AUTO_SCROLL_INTERVAL = 50;
let WA = BA;
class ZA {
  ...
  constructor(A = {}) {
    this.unicode = {
      ...
      if (g.preventDefault(), g.stopPropagation(), !(this.customWheelEventHandler && this.customWheelEventHandler(g)))
        if (((E = this.wasmTerm) == null ? void 0 : E.isAlternateScreen()) ?? !1) {
          const D = g.deltaY > 0 ? "down" : "up", o = Math.min(Math.abs(Math.round(g.deltaY / 33)), 5);
          for (let s = 0; s < o; s++)
            D === "up" ? this.dataEmitter.fire("\x1B[A") : this.dataEmitter.fire("\x1B[B");
        } else {
          let D;
          if (g.deltaMode === WheelEvent.DOM_DELTA_PIXEL) {
            const o = ((I = (C = this.renderer) == null ? void 0 : C.getMetrics()) == null ? void 0 : I.height) ?? 20;
            D = g.deltaY / o;
          } else
      ...
      if (E === 0)
        return;
      const C = this.canvas.getBoundingClientRect(), I = g.clientX - C.left, D = g.clientY - C.top, o = C.width, s = C.height, t = 8, e = o - t - 4, N = 4;
      if (I >= e && I <= e + t) {
        g.preventDefault(), g.stopPropagation(), g.stopImmediatePropagation();
        const w = s - N * 2, M = this.rows, G = E + M, r = Math.max(20, M / G * w), a = this.viewportY / E, K = N + (w - r) * (1 - a);
        if (D >= K && D <= K + r)
          ...
          const k = 1 - (D - N) / w, h = Math.round(k * E);
    ...
    }), this.cols = this.options.cols, this.rows = this.options.rows, this.buffer = new dA(this);
  }
  // ==========================================================================
  ...
  open(A) {
    ...
    try {
      ...
      this.canvas.addEventListener("mousedown", (E) => {
        ...
        E.preventDefault(), g.focus();
      }), this.renderer = new VA(this.canvas, {
        fontSize: this.options.fontSize,
        fontFamily: this.options.fontFamily,
        cursorStyle: this.options.cursorStyle,
        cursorBlink: this.options.cursorBlink,
        theme: this.options.theme
      }), this.renderer.resize(this.cols, this.rows), this.inputHandler = new bA(
        this.ghostty,
        A,
        (E) => {
...
`, B);
  ...
  processMouseMove(A) {
    ...
    if (w > 0)
      ...
      e = t + E;
    this.linkDetector.getLinkAt(g, e).then((M) => {
      var G, r, a, K;
      if (M !== this.currentHoveredLink && ((r = (G = this.currentHoveredLink) == null ? void 0 : G.hover) == null || r.call(G, !1), this.currentHoveredLink = M, (a = M == null ? void 0 : M.hover) == null || a.call(M, !0), this.element && (this.element.style.cursor = M ? "pointer" : "text"), this.renderer))
        if (M) {
          const k = ((K = this.wasmTerm) == null ? void 0 : K.getScrollbackLength()) || 0, h = this.getViewportY(), T = Math.max(0, Math.floor(h)), l = M.range.start.y - k + T, d = M.range.end.y - k + T;
          l < this.rows && d >= 0 ? this.renderer.setHoveredLinkRange({
  ...
  processScrollbarDrag(A) {
    ...
    if (B === 0)
      return;
    const g = this.canvas.getBoundingClientRect(), E = A.clientY - g.top - this.scrollbarDragStart, C = g.height - 8, I = this.rows, D = B + I, o = Math.max(20, I / D * C), s = -E / (C - o), t = Math.round(s * B), e = this.scrollbarDragStartViewportY + t;
    this.scrollToLine(Math.max(0, Math.min(B, e)));
  }
  /**
...
class _A {
  ...
  proposeDimensions() {
    ...
    if (typeof E.clientWidth > "u")
      return;
    const C = window.getComputedStyle(E), I = Number.parseInt(C.getPropertyValue("padding-top")) || 0, D = Number.parseInt(C.getPropertyValue("padding-bottom")) || 0, o = Number.parseInt(C.getPropertyValue("padding-left")) || 0, s = Number.parseInt(C.getPropertyValue("padding-right")) || 0, t = E.clientWidth, e = E.clientHeight;
    if (t === 0 || e === 0)
      return;
    const N = t - o - s - PA, w = e - I - D, M = Math.max(jA, Math.floor(N / g.width)), G = Math.max(vA, Math.floor(w / g.height));
...
function yA(Q) {
  if (!u(Q))
    throw new Error(`Message is expected to be an object, but got ${O(Q)}`);
  if (!Q.type)
    throw new Error("Message is missing the 'type' property");
  if (typeof Q.type != "string")
    throw new Error(`Message is expects the 'type' property to be a string, but got ${O(Q.type)}`);
  switch (Q.type) {
    case n.ConnectionInit:
    case n.ConnectionAck:
    case n.Ping:
    case n.Pong: {
      if (Q.payload != null && !u(Q.payload))
        throw new Error(`"${Q.type}" message expects the 'payload' property to be an object or nullish or missing, but got "${Q.payload}"`);
      break;
    }
    case n.Subscribe: {
      if (typeof Q.id != "string")
        throw new Error(`"${Q.type}" message expects the 'id' property to be a string, but got ${O(Q.id)}`);
      if (!Q.id)
        throw new Error(`"${Q.type}" message requires a non-empty 'id' property`);
      if (!u(Q.payload))
        throw new Error(`"${Q.type}" message expects the 'payload' property to be an object, but got ${O(Q.payload)}`);
      if (typeof Q.payload.query != "string")
        throw new Error(`"${Q.type}" message payload expects the 'query' property to be a string, but got ${O(Q.payload.query)}`);
      if (Q.payload.variables != null && !u(Q.payload.variables))
        throw new Error(`"${Q.type}" message payload expects the 'variables' property to be a an object or nullish or missing, but got ${O(Q.payload.variables)}`);
      if (Q.payload.operationName != null && O(Q.payload.operationName) !== "string")
        throw new Error(`"${Q.type}" message payload expects the 'operationName' property to be a string or nullish or missing, but got ${O(Q.payload.operationName)}`);
      if (Q.payload.extensions != null && !u(Q.payload.extensions))
        throw new Error(`"${Q.type}" message payload expects the 'extensions' property to be a an object or nullish or missing, but got ${O(Q.payload.extensions)}`);
      break;
    }
    case n.Next: {
      if (typeof Q.id != "string")
...
function gQ(Q, A) {
  return yA(typeof Q == "string" ? JSON.parse(Q, A) : Q);
}
function j(Q, A) {
  return yA(Q), JSON.stringify(Q, A);
}
var V = function(Q) {
  ...
  function I(w) {
    return function(M) {
      return Promise.resolve(M).then(w, e);
    };
  }
  function D(w, M) {
    g[w] && (E[w] = function(G) {
      return new Promise(function(r, a) {
        C.push([w, G, r, a]) > 1 || o(w, G);
      });
    }, M && (E[w] = M(E[w])));
  }
  function o(w, M) {
  ...
  function s(w) {
    w.value instanceof V ? Promise.resolve(w.value.v).then(t, e) : N(C[0][2], w);
  }
  function t(w) {
    o("next", w);
  }
  ...
  function N(w, M) {
    w(M), C.shift(), C.length && o(C[0][0], C[0][1]);
  }
};
function CQ(Q) {
  const {
  ...
  const k = K, h = (() => {
    const F = /* @__PURE__ */ (() => {
      ...
      return {
        ...
        emit(J) {
          var U;
          "id" in J && ((U = c[J.id]) === null || U === void 0 || U.call(c, J));
        }
      };
    })(), q = {
  ...
  let l, d = 0, tA, W = !1, X = 0, MA = !1;
  async function aA() {
    clearTimeout(tA);
    const [F, q] = await (l ?? (l = new Promise((U, S) => (async () => {
      if (W) {
        if (await t(X), !d)
          return l = void 0, S({ code: 1e3, reason: "All Subscriptions Gone" });
        X++;
      }
      h.emit("connecting", W);
      const y = new k(typeof A == "function" ? await A() : A, BQ);
      let b, Z;
      function _() {
        isFinite(I) && I > 0 && (clearTimeout(Z), Z = setTimeout(() => {
          y.readyState === k.OPEN && (y.send(j({ type: n.Ping })), h.emit("ping", !1, void 0));
        }, I));
      }
      T((L) => {
        l = void 0, clearTimeout(b), clearTimeout(Z), S(L), L instanceof FA && (y.close(4499, "Terminated"), y.onerror = null, y.onclose = null);
      }), y.onerror = (L) => h.emit("error", L), y.onclose = (L) => h.emit("closed", L), y.onopen = async () => {
        try {
          h.emit("opened", y);
          const L = typeof B == "function" ? await B() : B;
          if (y.readyState !== k.OPEN)
            return;
          y.send(j(L ? {
            type: n.ConnectionInit,
            payload: L
          } : {
            type: n.ConnectionInit
            // payload is completely absent if not provided
          }, r)), isFinite(o) && o > 0 && (b = setTimeout(() => {
            y.close(p.ConnectionAcknowledgementTimeout, "Connection acknowledgement timeout");
          }, o)), _();
        } catch (L) {
          h.emit("error", L), y.close(p.InternalClientError, GA(L instanceof Error ? L.message : new Error(L).message, "Internal client error"));
        }
      };
      let z = !1;
      y.onmessage = ({ data: L }) => {
        try {
          const R = gQ(L, a);
          if (h.emit("message", R), R.type === "ping" || R.type === "pong") {
            h.emit(R.type, !0, R.payload), R.type === "pong" ? _() : D || (y.send(j(R.payload ? {
              type: n.Pong,
              payload: R.payload
            } : {
              type: n.Pong
              // payload is completely absent if not provided
            })), h.emit("pong", !1, R.payload));
            return;
          }
          if (z)
            return;
          if (R.type !== n.ConnectionAck)
            throw new Error(`First message cannot be of type ${R.type}`);
          clearTimeout(b), z = !0, h.emit("connected", y, R.payload, W), W = !1, X = 0, U([
            y,
            new Promise((KQ, UA) => T(UA))
          ]);
        } catch (R) {
          y.onmessage = null, h.emit("error", R), y.close(p.BadResponse, GA(R instanceof Error ? R.message : new Error(R).message, "Bad response"));
        }
      };
    })())));
    F.readyState === k.CLOSING && await q;
    let c = () => {
    };
    const J = new Promise((U) => c = U);
    return [
      F,
      c,
      Promise.race([
        ...
        J.then(() => {
          if (!d) {
            const U = () => F.close(1e3, "Normal Closure");
            isFinite(C) && C > 0 ? tA = setTimeout(() => {
              F.readyState === k.OPEN && U();
            }, C) : U();
          }
        }),
        // or
  ...
  function eA(F) {
    if (DA(F) && (IQ(F.code) || [
      ...
      p.InternalClientError,
      p.BadRequest,
      p.BadResponse,
      p.Unauthorized,
      // CloseCode.Forbidden, might grant access out after retry
      p.SubprotocolNotAcceptable,
      // CloseCode.ConnectionInitialisationTimeout, might not time out after retry
      // CloseCode.ConnectionAcknowledgementTimeout, might not time out after retry
      p.SubscriberAlreadyExists,
      p.TooManyInitialisationRequests
  ...
  function NA(F, q) {
    ...
    return (async () => {
      for (d++; ; )
        try {
          const [y, b, Z] = await aA();
          if (J)
            return b();
          const _ = h.onMessage(c, (z) => {
            switch (z.type) {
              case n.Next: {
                q.next(z.payload);
                return;
          ...
          y.send(j({
            ...
            !J && y.readyState === k.OPEN && y.send(j({
              id: c,
              type: n.Complete
            }, r)), d--, J = !0, b();
          }, await Z.finally(_);
    ...
    iterate(F) {
      const q = [], c = {
        ...
        return EQ(this, arguments, function* () {
          for (; ; ) {
            for (q.length || (yield V(new Promise((b) => c.resolve = b))); q.length; )
              yield yield V(q.shift());
            if (c.error)
              throw c.error;
            if (c.done)
      ...
      return U.throw = async (S) => (c.done || (c.done = !0, c.error = S, c.resolve()), { done: !0, value: void 0 }), U.return = async () => (J(), { done: !0, value: void 0 }), U;
    },
    async dispose() {
      if (MA = !0, l) {
        const [F] = await l;
...
function DA(Q) {
  return u(Q) && "code" in Q && "reason" in Q;
}
function IQ(Q) {
  return [
    1e3,
    ...
    { query: Q, variables: A },
    {
      next: (I) => {
        ...
        if ((D = I.errors) != null && D.length) {
          g(new Error(((o = I.errors[0]) == null ? void 0 : o.message) ?? "Subscription error"));
          return;
        }
        I.data && B(I.data);
      },
...
const wQ = {
  ...
  style: { border: "1px solid var(--aurora-border-strong)", "border-radius": "var(--aurora-radius-2)", background: "var(--aurora-page-bg)", "box-shadow": "var(--aurora-shadow-strong)" }
}, tQ = {
  class: "flex shrink-0 items-center gap-2.5",
  style: { height: "42px", padding: "0 14px", "border-bottom": "1px solid var(--aurora-border-default)", background: "var(--aurora-panel-strong)" }
}, MQ = {
  width: "10",
  height: "14",
  viewBox: "0 0 10 14",
  ...
  style: { color: "var(--aurora-text-muted)", "font-size": "11px", "font-weight": "560" }
}, NQ = {
  key: 0,
  ...
  class: "p-4",
  style: { background: "var(--aurora-page-bg)" }
}, kQ = `
...
`, FQ = "mutation($sessionId: String!) { stopJailExec(sessionId: $sessionId) }", JQ = "subscription($sessionId: String!) { jailExecOutput(sessionId: $sessionId) }", UQ = /* @__PURE__ */ rA({
  __name: "Terminal",
  props: {
  ...
  emits: ["close"],
  setup(Q, { emit: A }) {
    const B = Q, g = A, E = IA(null), C = IA(null), I = IA("connecting");
    let D = !1, o = null, s = null, t = null, e = null, N = null, w = null;
    function M() {
      const k = getComputedStyle(document.documentElement), h = (l) => k.getPropertyValue(l).trim(), T = h("--aurora-accent-primary");
      return {
        background: h("--aurora-page-bg"),
        foreground: h("--aurora-text-primary"),
        cursor: T,
        selectionBackground: `color-mix(in srgb, ${T} 22%, transparent)`,
        black: h("--aurora-page-bg"),
        red: h("--aurora-error"),
        green: h("--aurora-success"),
        yellow: h("--aurora-warn"),
        blue: h("--aurora-info"),
        magenta: h("--aurora-accent-pink"),
        cyan: T,
        white: h("--aurora-text-primary"),
        brightBlack: h("--aurora-text-muted"),
        brightRed: h("--aurora-error-lift"),
        brightGreen: h("--aurora-success"),
        brightYellow: h("--aurora-warn"),
        brightBlue: h("--aurora-info"),
        brightMagenta: h("--aurora-accent-pink-strong"),
        brightCyan: h("--aurora-accent-strong"),
        brightWhite: "#ffffff"
      };
    }
    async function G() {
      if (E.value) {
        await $A(), o = new ZA({
          ...
          theme: M()
        }), s = new _A(), o.loadAddon(s), o.open(E.value), s.fit(), s.observeResize();
        try {
          t = (await $(kQ, {
            name: B.jailName,
            cols: o.cols,
          ...
          C.value = k instanceof Error ? k.message : String(k), I.value = "disconnected";
          return;
        }
        I.value = "connected", e = iQ(
          JQ,
          { sessionId: t },
          (k) => o == null ? void 0 : o.write(k.jailExecOutput),
          (k) => {
            C.value = k instanceof Error ? k.message : String(k), I.value = "disconnected";
          }
        ), N = o.onData((k) => {
          t && $(cQ, { sessionId: t, data: k }).catch(r);
        }), w = o.onResize(({ cols: k, rows: h }) => {
          t && $(GQ, { sessionId: t, cols: k, rows: h }).catch(r);
        });
      }
    }
    function r(k) {
      D || (D = !0, C.value = k instanceof Error ? k.message : String(k), I.value = "disconnected");
    }
    async function a() {
      N == null || N.dispose(), w == null || w.dispose(), s == null || s.dispose(), e == null || e(), o == null || o.dispose(), t && await $(FQ, { sessionId: t }).catch(r);
    }
    qA(G), nA(a);
    function K() {
      g("close");
    }
    return (k, h) => (gA(), EA("div", wQ, [
      H("div", sQ, [
        H("div", tQ, [
          (gA(), EA("svg", MQ, [...h[0] || (h[0] = [
            H("path", {
              d: "M5 0L9 2.5L5 5L1 2.5Z",
              fill: "var(--aurora-accent-primary)",
              opacity: "0.95"
            }, null, -1),
            H("path", {
              d: "M5 3L9 5.5L5 8L1 5.5Z",
              fill: "var(--aurora-accent-primary)",
              opacity: "0.75"
            }, null, -1),
            H("path", {
              d: "M5 6L9 8.5L5 11L1 8.5Z",
              fill: "var(--aurora-accent-primary)",
              opacity: "0.5"
            }, null, -1),
            H("path", {
              d: "M5 9L9 11.5L5 14L1 11.5Z",
              fill: "var(--aurora-accent-primary)",
              opacity: "0.28"
            }, null, -1)
          ])])),
          H("span", aQ, CA(Q.jailName), 1),
          h[2] || (h[2] = H("span", { style: { padding: "3px 7px", border: "1px solid var(--aurora-accent-primary-border)", "border-radius": "4px", background: "var(--aurora-accent-primary-surface)", color: "var(--aurora-accent-primary-foreground)", "font-family": "var(--aurora-font-mono)", "font-size": "10.5px", "font-weight": "600" } }, "ghostty-web", -1)),
          H("div", eQ, [
            H("span", {
              style: YA([{ width: "7px", height: "7px", "border-radius": "999px" }, {
                background: I.value === "connected" ? "var(--aurora-success)" : I.value === "connecting" ? "var(--aurora-warn)" : "var(--aurora-error)",
                boxShadow: `0 0 7px ${I.value === "connected" ? "var(--aurora-success)" : I.value === "connecting" ? "var(--aurora-warn)" : "var(--aurora-error)"}`
              }])
            }, null, 4),
            H("span", null, CA(I.value === "connected" ? "Connected" : I.value === "connecting" ? "Connecting…" : "Disconnected"), 1)
          ]),
          RA(SA(OA), {
            size: "sm",
            variant: "outline",
        ...
        C.value ? (gA(), EA("div", NQ, CA(C.value), 1)) : HA("", !0),
        H("div", hQ, [
          H("div", {
            ref_key: "containerEl",
            ref: E,
