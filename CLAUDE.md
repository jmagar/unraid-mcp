...
- **`unraid`** — primary tool with `action` (domain) + `subaction` (operation) routing, 17 actions / ~160 subactions. Call it as `unraid(action="docker", subaction="list")`.
...
| **user** (1) | me |
| **live** (16) | cpu, memory, cpu_telemetry, array_state, parity_progress, ups_status, notifications_overview, notifications_warnings, notification_feed, log_tail, owner, server_status, display, plugin_install_updates |
